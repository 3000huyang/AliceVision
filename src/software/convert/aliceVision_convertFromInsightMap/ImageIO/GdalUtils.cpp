...
void wGeo2Raster(const double *pTransform,double *x,double *y)
{
	double X = *x;
	double Y = *y;
	double a,b;const double *T=pTransform;
...
void wStatisticRaster(const char* lpstrFilePath,double *MinVal,double *MaxVal,double *MeanVal/*=NULL*/,double *StdDev/*=NULL*/)
{
	...
	if(hSrcDS)
	{
		...
		if(hBand)
		{
			if(MeanVal||StdDev){GDALComputeRasterStatistics(hBand,0,MinVal,MaxVal,MeanVal,StdDev,NULL,NULL);}
			else
			{
				double MinMax[2];
...
int GdalUtils::Force32bit(int w)
{
	...
	return fillW;
	//24bit 最后一定是按照32bitd对齐，求24和32最小公倍数96 / 24 = 4 表示四个像素
}
	...
	int &sw, int &sh, std::vector<uchar> &outRgbImages,int *pix_type)
{
	...
	for (int lev = 0; lev < info.Levels(); ++lev)
	{
		...
		if (std::max(ww, hh) > maxDimension){
			...
			if (!resetBuffer(*pix_type, ww, hh, buffer))
			{
				return false;
			}
	...
	if (!bRead)
	{
		...
		if (!resetBuffer(*pix_type, sw, sh, buffer))
		{
			return false;
		}
	...
	if (bRead) outRgbImages.swap(buffer);
	stream.Close();
	return bRead;
}
...
void GdalUtils::RgbToGray(const unsigned char *src, int n, unsigned char *des)
{
	for (int i = 0; i < n; ++i)
	{
		des[i] = src[i * 3 + 0] * 0.2126 + src[i * 3 + 1] * 0.7152 + src[i * 3 + 2] * 0.0722;
	}
}

void GdalUtils::ConvertRGBByteToGrayImage(const std::vector<unsigned char> &ptr, int w, int h, Image<unsigned char> * im)
{
	RGBColor * ptrCol = reinterpret_cast<RGBColor*>((unsigned char*)&ptr[0]);
...
void GdalUtils::MapRGBByteToImage(const std::vector<unsigned char> &ptr, int w, int h, Image<RGBColor> &rgbColIm)
{
	RGBColor * ptrCol = reinterpret_cast<RGBColor*>((unsigned char*)&ptr[0]);
	rgbColIm = Eigen::Map<Image<RGBColor>::Base>(ptrCol, h, w);
}

void GdalUtils::MapGrayByteToImage(const std::vector<unsigned char> &ptr, int w, int h, Image<uint8_t> &rgbColIm)
{
	//RGBColor * ptrCol = reinterpret_cast<RGBColor*>((unsigned char*)&ptr[0]);
	unsigned char * ptrCol = const_cast<unsigned char *>(ptr.data());
	rgbColIm = Eigen::Map<Image<unsigned char>::Base>(ptrCol, h, w);
}
