...
#ifndef INSIGHT_IMAGE_IMAGE_RESAMPLING_HPP_
#define INSIGHT_IMAGE_IMAGE_RESAMPLING_HPP_
...
INSIGHT_NAME_SPACE
  ...
  void ImageMapTo(const ImageT &src, const Image<Vec2f> &mapping, ImageT &out)
  {
	  const int new_width = src.Width();
	  const int new_height = src.Height();
	  ...
	  for (int i = 0; i < new_height; ++i)
	  {
		  for (int j = 0; j < new_width; ++j)
		  {
			  const Vec2f &vec = mapping(i, j);
			  if (vec[0] != -1 && vec[1] != -1){
				  out(i, j) =
					  SampleLinear(src, vec[0], vec[1]);
			  }
		  }
	  }
  }
